...
namespace virtual_receptionist.Models.Data
{
    ...
    public class BookingRepository : Repository, IManipulable<Booking>
    {
        ...
        {
            string sql =
                $"UPDATE booking SET booking.GuestID = (SELECT guest.Name FROM guest WHERE guest.Name = \"{booking.Guest.Name}\"), booking.RoomID = (SELECT room.Number FROM room WHERE room.ID = \"{booking.Room.Number}\"), NumberOfGuests = \"{booking.NumberOfGuests}\", ArrivalDate = \"{booking.ArrivalDate}\", DepartureDate = \"{booking.DepartureDate}\" WHERE booking.ID = \"{booking.ID}\"";
            database.DML(sql);
        }
